            ...
            <div className="industries-cards">
                {indutries.map((arr, index) => (
                    arr.map((industry, subIndex) => (
                        <div key={`${index}-${subIndex}`} className="industry-card">
                            <h3>{industry}</h3>
                        </div>
                    ))
                ))}
...
const WhyShelfLabSection = () => {
    const columns = ["Algorithm", "Interactive Demos", "Business Solutions"];

    return (
        <section className="why-shelflab-section">
            <h2>Why ShelfLab?</h2>
            <div className="why-shelflab-columns">
                {columns.map((column, index) => (
                    <div key={index} className="why-shelflab-column">
                        <h3>{column}</h3>
                    </div>
                ))}
            </div>
            ...
            <pre>
                <p>Unlike traditional software companies</p>
                <p>ShelfLab combines research</p>
                <p>interactive visualization</p>
                <p>and consulting to deliver</p>
                <p>algorithm-driven solutions.</p>
            </pre>
        </section>
    )
}
...
const CTASection = () => (
  <section className="cta-section">
    ...
    <pre>
      Whether you're building a startup, optimizing operations, or researching
      new ideas, ShelfLab is ready to help.
        </pre>
        <Link
